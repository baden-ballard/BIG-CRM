...
export async function GET(request: NextRequest) {
  ...
  // #region agent log
  fetch('http://127.0.0.1:7242/ingest/3a6a5ac4-a463-4d1c-82bb-202cb212287a',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'app/api/auth/confirm/route.ts:50',message:'Checking verification mode',data:{isDirectVerify,hasTokenHash:!!token_hash,hasType:!!type,userAgent:userAgent.substring(0,50)},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'A'})}).catch(()=>{});
  // #endregion
